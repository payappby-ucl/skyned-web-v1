...
import { financialAids } from "./constants";
...
const inEligibilityResponse = {
  isEligible: false,
  message:
    "Unfortunately, based on your current details, you're not eligible for a loan from either MPOWER or Passage. This may be due to your country of origin, school, or program eligibility. Please contact our team for alternate support options.",
};

export const computeFinancialAidEligibility = (
  program: IProgram,
  ...
  let supportedPartners = [
    "mpower",
    "passage",
  ] as (typeof financialAids)[number][];
  ...
  supportedPartners = supportedPartners.filter((partner) =>
    program.financialAids.includes(partner),
  );

  //   * Filter out passage and mpower ineligible citizenship
  if (passageInEligibleCitizenship.has(data.citizenship)) {
    supportedPartners = supportedPartners.filter(
      (partner) => partner !== "passage",
    );
  }

  if (mPowerInEligibleCitizenship.has(data.citizenship)) {
    supportedPartners = supportedPartners.filter(
      (partner) => partner !== "mpower",
    );
  }
  ...
  // * Partner check
  if (!supportedPartners.length) return inEligibilityResponse;

  //  * Compute recommendation
  let recommendation: (typeof financialAids)[number] | null = null;
