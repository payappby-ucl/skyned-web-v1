"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
var __exportStar = (this && this.__exportStar) || function(m, exports) {
    for (var p in m) if (p !== "default" && !Object.prototype.hasOwnProperty.call(exports, p)) __createBinding(exports, m, p);
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FirebaseServer = void 0;
const admin = __importStar(require("firebase-admin"));
const lib_1 = require("../lib");
const test_service_account_json_1 = __importDefault(require("../test-service-account.json"));
__exportStar(require("./interface"), exports);
class FirebaseServer {
    static instance = null;
    constructor(environment) {
        if (!admin.apps.length) {
            if (environment) {
                admin.initializeApp({
                    credential: admin.credential.cert(test_service_account_json_1.default),
                    storageBucket: "skyned-test-31a2e.firebasestorage.app",
                    projectId: "skyned-test-31a2e",
                });
                process.env.FIRESTORE_EMULATOR_HOST = lib_1.FIRESTORE_EMULATOR_HOST;
                process.env.FIREBASE_STORAGE_EMULATOR_HOST =
                    lib_1.FIREBASE_STORAGE_EMULATOR_HOST;
                process.env.FIREBASE_AUTH_EMULATOR_HOST = lib_1.FIREBASE_AUTH_EMULATOR_HOST;
            }
            else {
                admin.initializeApp();
            }
        }
    }
    static factory(environment) {
        if (!FirebaseServer.instance) {
            FirebaseServer.instance = new FirebaseServer(environment);
        }
        return FirebaseServer.instance;
    }
}
exports.FirebaseServer = FirebaseServer;
